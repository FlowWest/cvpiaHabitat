...
cvpiaHabitat::apply_suitability(
  cvpiaHabitat::square_meters_to_acres(
    cvpiaHabitat::set_floodplain_habitat(watershed = 'Feather River', species = 'fr', flow = 5000)))
...
fp_threshold_flow <- cvpiaHabitat::feather_river_floodplain$flow_cfs[which(cumsum(cvpiaHabitat::feather_river_floodplain$FR_floodplain_acres != 0) == 1)]
...
days_inundated <- feather %>%
  ...
  mutate(fp_area_acres = cvpiaHabitat::apply_suitability(
    cvpiaHabitat::square_meters_to_acres(
      cvpiaHabitat::set_floodplain_habitat(watershed = 'Feather River', species = 'fr', flow = monthly_mean_flow))))
