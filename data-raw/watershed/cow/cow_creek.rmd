...
There were no watershed specific instream spawning habitat data available for Cow Creek. Instream spawning data for Cow Creek were instead developed with habitat modeling using a regional approximation of similar watersheds in the Upper-mid Sacramento River Region. The data were then used in the Decision Support Model (DSM). 
...
### Rearing WUA
The following plot shows the weighted usable rearing area (WUA) in square feet per thousand feet of river for Fall Run Chinook Salmon fry and juvenile. These rates are multiplied by the total rearing reach length mapped by the Science Integration Team (SIT) to estimate available habitat. 
``` {r,echo=FALSE}
cow_creek_instream %>% 
