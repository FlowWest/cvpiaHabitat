...
**Data Source:**
[FWS 2003](https://www.fws.gov/lodi/instream-flow/Documents/Butte%20Creek%20Spring-run%20chinook%20salmon%20spawning%208-29-2003.pdf)
[FERC relicensing of DeSabla](https://s3-us-west-2.amazonaws.com/cvpiahabitat-r-package/cvpia-sit-model-inputs/DeSabla2008ButteIFIM.pdf) 
...
spawning <- read.csv('data-raw/watershed/butte/data/butte_creek_data.csv', 
                     skip = 1) 

# Combined unique flow values  
flow <- spawning %>% 
  pull(flow_cfs) %>%
  unique() %>%
  sort()
...
make_approx_fun(spawning)

butte_creek_spawning <- tibble(
  flow_cfs = flows, 
  SP_spawn_wua =
    make_approx_fun(spawning)(flow_cfs) * reach_miles
  )
