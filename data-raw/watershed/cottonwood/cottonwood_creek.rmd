...
cotton_reach_1 <- read_excel('data-raw/watershed/cottonwood/data/cottonwood_combined.xlsx', range = "G4:K17")%>%
  mutate(mi = 8.99)
cotton_reach_2 <- read_excel('data-raw/watershed/cottonwood/data/cottonwood_combined.xlsx', range = "G21:K34")%>%
  mutate(mi = 13.42)
cotton_reach_3 <- read_excel('data-raw/watershed/cottonwood/data/cottonwood_combined.xlsx', range = "G38:K47")%>%
  mutate(mi = 8.84)
...
```
```{r, echo = FALSE}
knitr::kable(align = 'c', head(cottonwood_creek_instream, 5), caption = "Header Descriptions: flow_cfs = flow in cubic feet per second, FR_spawn_wua = Fall Run Chinook Spawning wua, FR_fry_wua = Fall Run Chinook Fry wua, FR_juv_wua = Fall Run Chinook Juvenile wua, ST_fry_wua = Steelhead Fry wua, ST_juv_wua = Steelhead Juvenile wua")
```
...
## Floodplain Data    
```{r, echo=FALSE}
cotton_fp <- read_excel('data-raw/watershed/CVPIA_FloodplainAreas.xlsx', sheet = 'CottonwoodCreek') %>%
  mutate(watershed = 'Cottonwood Creek')
...
cottonwood_creek_floodplain <- scale_fp_flow_area_partial_model(ws = 'Cottonwood Creek', df = filter(cotton_fp, flow_cfs >= threshold_flow))

knitr::kable(align = 'c', head(cottonwood_creek_floodplain, 5), caption = "Header Descriptions: flow_cfs = flow in cubic feet per second, FR_floodplain_acres = Fall Run Chinook floodplain acres, SR_floodplain_acres = Spring Run Chinook floodplain acres, ST_floodplain_acres = Steelhead floodplain acres")
