...
```{r, echo =FALSE}
total <- 71632
calaveras_reach_1and2 <- read_excel('data-raw/watershed/calaveras/Calaveras_all_reaches.xlsx', range = "A16:D26") %>%
  mutate(ft = 18282)%>%
  rename(flow_cfs = "Flow", ST_fry_wua = 'Fry Rearing', ST_juv_wua = 'Juv Rearing', ST_spawn_wua = 'Spawning') 
calaveras_reach_3 <- read_excel('data-raw/watershed/Calaveras/Calaveras_all_reaches.xlsx', range = "A30:D40") %>%
  mutate(ft = 25438)%>%
  rename(flow_cfs = "Flow", ST_fry_wua = 'Fry Rearing', ST_juv_wua = 'Juv Rearing', ST_spawn_wua = 'Spawning') 
calaveras_reach_4 <- read_excel('data-raw/watershed/calaveras/Calaveras_all_reaches.xlsx', range = "A43:D53") %>%
  mutate(ft = 27867)%>%
  rename(flow_cfs = "Flow", ST_fry_wua = 'Fry Rearing', ST_juv_wua = 'Juv Rearing', ST_spawn_wua = 'Spawning') 
...
knitr::kable(head(calaveras_river_instream, 5), align = "c", caption = "Header Descriptions: flow_cfs = flow in cubic feet per second, ST_spawn_wua = Steelhead spawning wua, ST_fry_wua = Steelhead Fry wua, ST_juv_wua = Steelhead Juvenile wua")
...
**Data Source:** [TUFLOW hydraulic model with 1D channel and 2D overbank components](https://s3-us-west-2.amazonaws.com/cvpiahabitat-r-package/cvpia-sit-model-inputs/Tuolumne_W-AR_21__Study+Report.pdf)
...
knitr::kable(align = 'c', head(calaveras_river_floodplain, 5), caption = "Header Descriptions: flow_cfs = flow in cubic feet per second, FR_floodplain_acres = Fall Run Chinook floodplain acres, ST_floodplain_acres = Steelhead floodplain acres")
