...
```
```{r, include=FALSE}
upper_fr <- read_csv('data-raw/watershed/feather/alt_upper_feather_spawn_wua_rsi.csv') %>%
  mutate(location = 'upper')
lower_fr <- read_csv('data-raw/watershed/feather/alt_lower_feather_spawn_wua_rsi.csv') %>%
  mutate(location = 'lower')
upper_st <- read_csv('data-raw/watershed/feather/alt_upper_feather_spawn_wua_rsi_steelhead.csv') %>%
  mutate(location = 'upper')
lower_st <- read_csv('data-raw/watershed/feather/alt_lower_feather_spawn_wua_rsi_steelhead.csv') %>%
  mutate(location = 'lower')
spawning <- tibble(
...
knitr::kable(align ="c", head(feather_river_instream, 5), caption = "Header Descriptions: flow_cfs = flow in cubic feet per second, FR_spawn_wua = Fall Run Spawning wua, ST_fry_wua = Steelhead Fry wua, ST_juv_wua = Steelhead Juvenile wua, ST_spawn_wua = Steelhead Spawning wua")
...
knitr::kable(align = 'c', head(feather_river_floodplain, 5), caption = "Header Descriptions: flow_cfs = flow in cubic feet per second, FR_floodplain_acres = Fall Run Chinook floodplain acres, SR_floodplain_acres = Spring Run Chinook floodplain acres, ST_floodplain_acres = Steelhead floodplain acres")
