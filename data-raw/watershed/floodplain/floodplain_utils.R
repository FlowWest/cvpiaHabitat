...
metadata <- read_excel('data-raw/watershed/floodplain/CVPIA_FloodplainAreas.xlsx', sheet = 'MetaData',
...
scale_fp_flow_area <- function(ws) {
  ...
  if (proxy_watershed == 'deer_creek') {
    temp_df <- read_excel('data-raw/watershed/floodplain/CVPIA_FloodplainAreas.xlsx', sheet = 'DeerCreek')
    proxy_watershed_metadata <- filter(metadata, watershed == 'Deer Creek')
  }

  if (proxy_watershed == 'cottonwood_creek') {
    temp_df <- read_excel('data-raw/watershed/floodplain/CVPIA_FloodplainAreas.xlsx', sheet = 'CottonwoodCreek')
    proxy_watershed_metadata <- filter(metadata, watershed == 'Cottonwood Creek')
  }

  if (proxy_watershed == 'tuolumne_river') {
    temp_df <- read_excel('data-raw/watershed/floodplain/CVPIA_FloodplainAreas.xlsx', sheet = 'TuolumneRiver')
