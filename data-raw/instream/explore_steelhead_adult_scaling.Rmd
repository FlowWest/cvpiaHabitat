...
devtools::use_data(prop_merced_flows)
...
```{r}
  if (life_stage == 'adult') {
    if (dplyr::pull(modeling_lookup, ST_adult)) {
      adult_col <- paste(toupper(species), life_stage, 'wua', sep = '_')
      return(approxfun(relationship_df$flow_cfs, relationship_df[adult_col], rule = 2))
    } else {
      subset(prop_merced_flows, watershed = 'Yuba River')$prop_merced
      return()
    }
  }
